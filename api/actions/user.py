...
async def _activate_user(user_id, session) -> UUID | None:
    async with session.begin():
        user_dal = UserDAL(session)
        activated_user_id = await user_dal.activate_user(user_id=user_id)
        return activated_user_id


async def _get_user_by_id(user_id, session) -> ShowUser | None:
    async with session.begin():
        user_dal = UserDAL(session)
...
async def _get_user_by_email(email, session) -> ShowUser | None:
    async with session.begin():
        user_dal = UserDAL(session)
...
async def _update_user(updated_user_params: dict, user_id: UUID, session) -> UUID | None:
    async with session.begin():
        user_dal = UserDAL(session)
